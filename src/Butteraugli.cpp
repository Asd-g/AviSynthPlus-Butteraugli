...
const double* NewSrgbToLinearTable()
{
	...
	for (int i = 0; i < 256; ++i)
	{
		...
		table[i] =	255.0 * (srgb <= 0.04045 ? srgb / 12.92	: std::pow((srgb + 0.055) / 1.055, 2.4));
	}

	return table;
} //end NewSrgbToLinearTable

static void FromSrgbToLinear(const std::vector<butteraugli::Image8>& rgb, std::vector<butteraugli::ImageF>& linear, int background)
{
